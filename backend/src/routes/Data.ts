...
router.post('/likeBlog',async(req:Request,res:Response<returnData>)=>{
...
router.post('/getBlogs',async(req:Request,res:Response<returnData>)=>{
    if(await verify(req.cookies.token,req.body.uuid) === false){
        return res.status(400).json({message:"Unauthorized",ok:false})
    }
...
router.post('/editBlog',async(req:Request,res:Response<returnData>)=>{
    if(await verify(req.cookies.token,req.body.uuid) === false){
        return res.status(400).json({message:"Unauthorized",ok:false})
    }
...
router.post('/addBlog',async(req:Request,res:Response<returnData>)=>{
    if(await verify(req.cookies.token,req.body.uuid) === false){
        return res.status(400).json({message:"Unauthorized",ok:false})
    }
...
router.delete('/deleteBlog',async(req:Request,res:Response<returnData>)=>{
    if(await verify(req.cookies.token,req.body.uuid) === false){
        return res.status(400).json({message:"Unauthorized",ok:false})
    }
